...
// import { getVideoList } from '@lgc_cms/actions/server';
import { Card, CardContent, CardFooter } from '@lgc_cms/shadcn-ui';
...
export function VideoLibraryList({
  ...
  return (
    <div
      className={'flex h-full flex-col justify-stretch overflow-y-auto px-5'}
    >
      ...
      <div className="video-library-list grid grid-cols-1 gap-3 md:grid-cols-4 lg:grid-cols-5">
        {videoLibraryList.map((video: any) => (
          <div key={video.guid} className="video-library-list__item">
            <Card
              className={
                'group flex h-full flex-col justify-stretch overflow-hidden'
              }
            >
              ...
              <CardContent className={'flex-1'}>
                <h3 className={'mt-4 text-wrap text-sm font-bold'}>
                  {video.title}
                </h3>
              </CardContent>
