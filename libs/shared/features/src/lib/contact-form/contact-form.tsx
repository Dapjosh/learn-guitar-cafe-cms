...
import { IContactFormInput } from '@lgc_cms/types';
...
// import { sendMail, VerifyCaptcha } from '@lgc_cms/actions/server';
...
const emailSchema = z.object({
  ...
  agreeToTerms: z
    .boolean()
    .refine((val) => val, 'You must agree to the Terms & Conditions'),
});
...
export function ContactForm({ items }: ContactFormInputProps) {
  ...
  if (submissionSuccess) {
    return (
      <div
        className={
          'container mx-auto flex max-w-2xl flex-col items-center justify-center'
        }
      >
  ...
  return (
    <div className="form-control text-primary w-full max-w-3xl">
      ...
      >
        ...
        {errors.enquiryType && (
          <p className="text-error">{errors.enquiryType.message}</p>
        )}
          ...
          <div className="w-full">
            <div className="flex items-center gap-2">
              <input
                type="checkbox"
                className={cn(
                  `checkbox ${errors.agreeToTerms ? 'checkbox-error' : ''}`,
                  'rounded-md'
                )}
